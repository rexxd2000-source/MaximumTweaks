    ...
    def _detect_changes(self, old: Route, new: Route) -> list[RouteChange]:
        ...
        if old_ips != new_ips:
            ...
            def _ip_nums(hops):
                ...
                for h in hops:
                    if h.ip:
                        m.setdefault(h.ip, []).append(h.number)
                return m

            old_nums = _ip_nums(old.hops)
            new_nums = _ip_nums(new.hops)

            removed_desc = ", ".join(
                f"{ip} (hop {old_nums[ip][0]})" for ip in sorted(removed)
            )
            added_desc = ", ".join(
                f"{ip} (hop {new_nums[ip][0]})" for ip in sorted(added)
            )
            if added_desc and removed_desc:
                why = f": removed {removed_desc}; added {added_desc}"
            elif added_desc:
                why = f": added {added_desc}"
            elif removed_desc:
                why = f": removed {removed_desc}"
            else:
                why = " (hop reorder)"
