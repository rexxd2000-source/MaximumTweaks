...
class TweaksPage(QWidget):
    ...
    def _build_header(self):
        # Reference .p-head: h1 + subtitle on the left, stat pill with two
        # mono stats on the right, separated from the rest by a bottom border.
        head = QFrame()
        head.setFixedHeight(HEADER_H)
        hl = QVBoxLayout(head)
        hl.setContentsMargins(0, 16, 0, 14)
        hl.setSpacing(0)
        top = QHBoxLayout()
        top.setSpacing(20)
        ...
        pill.setStyleSheet(
            f"QFrame{{background:{T['card_alt']}; border:1px solid {T['border']};}}")
    ...
    def _dot_stat(color: str, number, label: str) -> str:
        return (f"<span style='color:{color};'>\u25cf</span>"
                f"<span style='color:{T['text']};font-weight:600;'>&nbsp;{number}</span>"
                f"<span style='color:{T['text_dim']}'>&nbsp;{label}</span>")
